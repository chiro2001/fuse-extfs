...
struct custom_options rfs_options;       /* 全局选项 */
struct rfs_super super;
/******************************************************************************
* SECTION: FUSE操作定义
*******************************************************************************/
 ...
 * @return void*
 */
void *rfs_init(struct fuse_conn_info *conn_info) {
  /* TODO: 在这里进行挂载 */
 ...
 * @return int 0成功，否则失败
 */
int rfs_utimens(const char *path, const struct timespec tv[2]) {
  (void) path;
  return 0;
}
/******************************************************************************
 ...
 * @return int 0成功，否则失败
 */
int rfs_access(const char *path, int type) {
  /* 选做: 解析路径，判断是否存在 */
  return 0;
}
